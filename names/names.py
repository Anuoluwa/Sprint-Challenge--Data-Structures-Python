import time

from collections import Counter, deque

start_time = time.time()

f = open('names_1.txt', 'r')
names_1 = f.read().split("\n")  # List containing 10000 names
f.close()

f = open('names_2.txt', 'r')
names_2 = f.read().split("\n")  # List containing 10000 names
f.close()

names_1 = Counter(names_1)
names_2 = Counter(names_2)

# this implementation uses other external resources online

# duplicates = [] # this will runtime: 0.02987384796142578 seconds

duplicates = deque() #runtime: 0.012484073638916016 seconds


for name in names_1:
    if name in names_2:
        duplicates.append(name)

# for name_1 in names_1:
#     for name_2 in names_2:
#         if name_1 == name_2:
#             duplicates.append(name_1)

end_time = time.time()
print (f"{len(duplicates)} duplicates:\n\n{', '.join(duplicates)}\n\n")
print (f"runtime: {end_time - start_time} seconds")

# ---------- Stretch Goal -----------
# Python has built-in tools that allow for a very efficient approach to this problem
# What's the best time you can accomplish with no restrictions on techniques or data
# structures?

# using this duplicates = deque() , we have #runtime: 0.012484073638916016 seconds using this above we have this,
# lists in python built in, # duplicates = [] # this will runtime: 0.02987384796142578 seconds
